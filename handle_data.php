...
if (isset($_REQUEST['action'])) {
    ...
    if ($action == "Get") {
        ...
        foreach ($observationID as $observID) {
            ...
            for($i=0;$i<5;$i++)
            {
                $line_add2=$result_Overviews->fetch_row();
                if($line_add2)
                {
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                if ($line_add2) {
                    $elements = explode(',', $line_add2[0]);  //note may contain comma, so enclose field with double quote
                    if (count($elements) > 3) {
                        $part1 = $elements[0];
                        $part3 = $elements[count($elements) - 1];
                        $part2 = "\"";
                        for ($i = 1; $i < count($elements) - 1; $i++) {
                            $part2.="$elements[$i]";
                        }
                        $part2.="\"";
                        $timeNotes[$i] = "$part1,$part2,$part3";
                    } else if ($elements == 3) {
                        $timeNotes[$i] = "$elements[0],$elements[1],$elements[2]";
                    } else {
                        while (count($elements) != 3) {
                            $elements[] = "";
                        }
                        $timeNotes[$i] = "$elements[0],$elements[1],$elements[2]";
                    }
...
function export_csv($filename,$dir) {
